...
class Builder
{
     ...
     * @return array|static[]
     */
    public function pageInfo($page, $perPage = 15, $total=0) {

        if (!$total) {
            $total = $this->getCountForPagination();
        }
        $page_info['size']        = (int)$perPage;
        $page_info['current']     = (int)$page;
        $page_info['total_rows']  = (int)$total;
        $page_info['total_pages'] = ceil($page_info['total_rows'] / $page_info['size']);

        if ($page_info['total_pages'] < $page_info['current']) {
            $page_info['current'] = $page_info['total_pages'];
        }
        if (2 > intval($page_info['current'])) {
            $page_info['current'] = 1;
        }

        $this->forPage($page_info['current'], $perPage);
        $rows  = $this->runSelect();

        return ['rows'=>$rows, 'pages'=>$page_info];
    }
     ...
     * Run the query as a "selectu" statement against the connection.
     *
     * @return array
     */
