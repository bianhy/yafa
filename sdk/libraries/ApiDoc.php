...
use SDK\Libraries\Http\Request;

class ApiDoc
{
    public static function parse($controller = 'index', $action = 'index')
    {
        $description = '';
        $descComment = '';
        ...
        $docComment    = $reflector->getMethod($action.'Action')->getDocComment();
        ...
        if ($example) {
            ...
            if (!$example_ret){
                $ret = Request::get($example_url);
                $example_ret = self::jsonFormat($ret);
            }
        }
        ...
        $service_name = $controller.'/'.$action;
        $api_desc_tpl = [
